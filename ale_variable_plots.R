...
files_num <- grep('ale_habitat_variable', dir("results"))
...
write.table(plotter, "results/ale_variable_coastal.csv", sep = ",",
            quote = FALSE, row.names = FALSE)

baseline <- mean(plotter$pop[plotter$upstream == 0])

# Plotting code
ale_coastal_variable <- ggplot(plotter, aes(x = upstream, y = pop, color = downstream, fill = downstream)) +
  # geom_line() +
  # geom_ribbon(
  ...
  ylab("Billions of spawners") +
  labs(color = "DS Adult",
       fill = "DS Adult") +
  scale_y_continuous(breaks = seq(0,20e9,.5e9),
...
ale_coastal_variable

jpeg("results/ale_variable_coastal.jpg",
     res = 300, 
     height = 2400,
     width = 3200)
ale_coastal_variable
...
write.table(plotter, "results/ale_variable_regional.csv", sep = ",",
            quote = FALSE, row.names = FALSE)
...
ale_variable_regional <- ggplot(
  ...
  ggh4x::facetted_pos_scales(y = list(
    region == "MAT" ~ scale_y_continuous(breaks = seq(0, 2e9,.05e9),
                                         labels = format(seq(0, 2, .05), digits = 2),
                                         limits = c(0, NA),
                                         expand = c(0, NA)),
    region == "SNE" ~ scale_y_continuous(breaks = seq(0, 2e9,.01e9),
                                         labels = format(seq(0, 2, .01), digits = 2),
                                         limits = c(0, NA),
                                         expand = c(0, NA)),
    region == "NNE" ~ scale_y_continuous(breaks = seq(0, 2e9, .5e9),
                                         labels = format(seq(0, 2, .5), digits = 2),
                                         limits = c(0, NA),
                                         expand = c(0, NA)))) +
  scale_x_continuous(breaks = seq(0, 1, .5)) +
  theme_bw() +
...
ale_variable_regional

jpeg("results/ale_variable_regional.jpg",
     res = 300, 
     height = 2400,
     width = 3200)
ale_variable_regional
